...
def plot_metrics(df, team1, team2=None, metrics =  ['FTHG', 'FTAG', 'HS', 'HST', 'HC', 'AS', 'AST', 'AC'], mapping={}, ax=None, raw_val=False):
    # Example dataset (replace with your actual dataset)
     
    # Select teams and stats to compare
    print("PLOT", team1, team2)
    if team2 == None:
        teams = [team1]
    else:
        teams = [team1, team2]
    ...
    if raw_val == True:
        ...
        normalised_stats = stats[stats_to_plot]

    else:
        # Normalise data to a range of [0, 1]
        normalised_stats = stats[stats_to_plot].apply(lambda x: x / x.max(), axis=0)
    ...
    for team in teams:
        ...
        values = normalised_stats.loc[team].values.flatten().tolist()
